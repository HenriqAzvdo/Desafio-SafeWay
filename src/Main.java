...
public class Main {
	...
	public static void main(String[] args) {
		...
		Produto produto5 = new Produto(5, "Prato feito - Frango", 10, 25.0d, empresa3);
		...
		empresas.sort(Comparator.comparing(Empresa::getId));
				...
				if ((usuarioLogado.getSenha().equals(senha))) {
					...
					if (usuarioLogado.IsEmpresa()) {
							...
							vendas.stream().forEach(venda -> {
								if (venda.getEmpresa().getId().equals(usuarioLogado.getEmpresa().getId())) {
									...
									System.out.println("Total Líquido  para empresa R$"
											+ (new DecimalFormat("0.00"). format((venda.getValor() - venda.getComissaoSistema())))) ;
									System.out.println("************************************************************");
								}

							});
							System.out.println("Saldo Empresa: R$" + (new DecimalFormat("0.00"). format(usuarioLogado.getEmpresa().getSaldo())));
							System.out.println("************************************************************");
						...
						case 2: {
							...
							produtos.stream().forEach(produto -> {
								if (produto.getEmpresa().getId().equals(usuarioLogado.getEmpresa().getId())) {
									...
									System.out.printf("Valor: R$ %.2f%n", produto.getPreco());			//aqui deu certo					
									System.out.println("************************************************************");
								}
						...
						case 1: {
							...
							System.out.println("Total: R$" + (new DecimalFormat("##.00"). format(venda.getValor())));
							System.out.println("************************************************************");
							carrinho.clear();
							executar(usuarios, clientes, empresas, produtos, carrinho, vendas);
						}
